...
def write_to_env_file(env_script, env):
    header("Write to .env")
    if os.path.exists(ENV_FILE):
        ...
        info("Cleaned old Infisical block from .env")
    with open(ENV_FILE, 'a') as f:
        f.write('\n' + env_script + '\n')
    ok(f"Appended secrets to {ENV_FILE}")
...
def main():
    ...
    parser.add_argument("--no-env", action="store_true", help="Skip .env file write")
    ...
    if args.dry_run:
        header("Dry Run - Generated .env Content")
        print(env_script)
        return
    if not args.no_env:
        write_to_env_file(env_script, args.env)
    if not args.no_ssh:
    ...
    print()
    ok("All done! Secrets written to .env file.")
    print()
